...
<img src="docs/inbox-dark.png" alt="The testmail inbox, showing a verification email with the code and sign-in link pulled out above the message" width="880">
...
testmail skips the reading: it pulls the **one-time code** and the **action link** out of the
message and hands you those.

```sh
testmail                  # test-k3f9qz1p@uberip.com  (copied)
# …paste it into your signup form…
testmail code --wait      # 482910  (copied)
```
...
Want to look around first? `TESTMAIL_DEMO=1 testmail ui` serves a fixed example inbox and
...
```sh
TOKEN=$(testmail wait 60 --json | jq -r '.links[0]')
curl -sS "$TOKEN"
```
...
mail.tm provides the mailbox. testmail keeps the account in `~/.config/testmail/account.json`,
...
- **mail.tm expires idle accounts.** testmail creates a new one for you, but the old messages
  are gone.
